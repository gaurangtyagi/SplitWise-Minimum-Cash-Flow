...
void minCashFlowRec(int amount[], int N)
{
	...
	// amount[mxCredit] indicates the maximum amount to be given to any person .
	// And amount[mxDebit] indicates the maximum amount to be taken from any person.
...
int main()
{
  // N indicates total number of people
  int N; 
  cin>>N; 
	
  // graph[i][j] indicates the amount that person i needs to
	// pay person j
	int graph[N][N]; // 0 - indexed
  // Initializing all the debts to 0; 
  for (int i = 0; i<N; i++)
  {
      for (int j = 0; j<N; j++)
      {
  ...
  while(true)
  {
      cout<<"Amount of money A owns to B is ";
      cin>>A>>B>>X;
      graph[A-1][B-1] = X; 
      cout<<"More transactions remaining? (enter -1 if no)";
      cin>>X; 
      if (X == -1) break; 
  }
	// Print the solution
